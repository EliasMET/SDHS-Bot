...
class Settings(commands.Cog, name="settings"):
    ...
    async def cog_load(self):
        """
        Initializes the database manager and caches the bot owner's ID.
        """
        ...
        try:
            ...
            self.bot.logger.info(f"Cached owner_id: {self.owner_id}")
    ...
    async def cog_unload(self):
        """
        Placeholder for any cleanup when the cog is unloaded.
        """
        self.bot.logger.info("Settings cog unloaded.")
    ...
    async def create_tryout_settings_embed(self, guild):
        ...
        roles_display = ", ".join([f"<@&{role_id}>" for role_id in required_roles]) if required_roles else "Not Set"

        tryout_groups = await self.db.get_tryout_groups(guild.id)
        groups_display = "\n".join([f"**{group[3]}** (Group ID: {group[0]})" for group in tryout_groups]) if tryout_groups else "No groups configured."

        ping_roles = await self.db.get_ping_roles(guild.id)
        ping_roles_display = ", ".join([f"<@&{role_id}>" for role_id in ping_roles]) if ping_roles else "No roles set."
    ...
    async def create_moderation_settings_embed(self, guild):
        ...
        roles_display = ", ".join([f"<@&{role_id}>" for role_id in allowed_roles]) if allowed_roles else "No roles set."
...
class BaseChannelModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if error:
            ...
            await interaction.response.send_message(embed=embed, ephemeral=True)
            self.settings_cog.bot.logger.warning(
        ...
        try:
            ...
            self.settings_cog.bot.logger.error(f"Error setting channel '{self.setting_name}' in guild {self.guild.id}: {e}")
            embed = discord.Embed(
                title="Error",
...
class BaseRoleManagementModal(discord.ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            self.settings_cog.bot.logger.error(f"Error managing roles in guild {self.guild.id}: {e}")
            embed = discord.Embed(
                title="Error",
...
class ManageTryoutGroupsModal(discord.ui.Modal, title="Manage Tryout Groups"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            self.settings_cog.bot.logger.error(f"Error managing tryout group '{group_id}' in guild {self.guild.id}: {e}")
            embed = discord.Embed(
                title="Error",
...
class AutomodSettingsView(discord.ui.View):
    ...
    async def update_settings_message(self):
        ...
        try:
            ...
            self.settings_cog.bot.logger.info(f"Updated Automod settings message in guild {self.guild.id}.")
        except Exception as e:
    ...
    async def on_timeout(self):
        ...
        self.settings_cog.bot.logger.info(f"AutomodSettingsView timed out in guild {self.guild.id}.")
...
class ModerationSettingsView(discord.ui.View):
    ...
    async def update_settings_message(self):
        ...
        try:
            ...
            await self.message.edit(embed=embed, view=self)
            self.settings_cog.bot.logger.info(f"Updated Moderation settings message in guild {self.guild.id}.")
        except Exception as e:
    ...
    async def on_timeout(self):
        ...
        if self.message:
            await self.message.edit(view=self)
        self.settings_cog.bot.logger.info(f"ModerationSettingsView timed out in guild {self.guild.id}.")
...
class TryoutSettingsView(discord.ui.View):
    ...
    async def update_settings_message(self):
        ...
        try:
            ...
            await self.message.edit(embed=embed, view=self)
            self.settings_cog.bot.logger.info(f"Updated Tryout settings message in guild {self.guild.id}.")
        except Exception as e:
    ...
    async def on_timeout(self):
        ...
        if self.message:
            await self.message.edit(view=self)
        self.settings_cog.bot.logger.info(f"TryoutSettingsView timed out in guild {self.guild.id}.")
