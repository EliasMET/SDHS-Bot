...
class AutoMod(commands.Cog):
    ...
    async def handle_message_violation(self, message: discord.Message, reason: str):
        try:
            ...
            warnings = await self.db.get_warnings(
            ...
            await moderation_message.delete(delay=10)
    ...
    async def on_message(self, message: discord.Message):
        ...
        if self.roblox_group_regex.search(message.content):
            await self.handle_message_violation(message, "Posting Roblox group links.")
        elif self.discord_invite_regex.search(message.content):
            await self.handle_message_violation(message, "Posting Discord invite links.")
        elif any(profanity in content_lower for profanity in self.profanity_list):
    ...
    async def expire_warnings_task(self):
        ...
        try:
            ...
            warnings = await self.db.get_all_warnings()  # Fetch all warnings
            for warn in warnings:
                created_at = datetime.fromtimestamp(int(warn[4]))
                if created_at < expiration_time:
                    await self.db.remove_warn(
                        warn_id=warn[5], user_id=warn[0], server_id=warn[1]
                    )
                    logger.info(f"Expired warning #{warn[5]} for user ID {warn[0]}.")
        except Exception as e:
            logger.error(f"Error in warning expiration task: {e}")

    @tasks.loop(count=1)
    async def load_profanity_list_task(self):
        """Load the profanity list from the provided URL."""
            ...
            async with aiohttp.ClientSession() as session:
                async with session.get(url) as response:
                    if response.status == 200:
                        ...
                        self.profanity_list = text.splitlines()
                        logger.info("Profanity list loaded successfully.")
                    else:
